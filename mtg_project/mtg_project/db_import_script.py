...
cards_json =    BASE_DIR + "/mtg_project/db_loading_json/cards_json.json"
sets_json =     BASE_DIR + "/mtg_project/db_loading_json/sets_json.json"
type_json =     BASE_DIR + "/mtg_project/db_loading_json/types_json.json"
subtype_json =  BASE_DIR + "/mtg_project/db_loading_json/subtypes_json.json"
type_pairs =    BASE_DIR + "/mtg_project/db_loading_json/types_subtypes.json"
...
def import_types () :
    ...
    for m_type in types_array :
        ...
        if m_type[0] in m_type_subtypes and t is not None and created:
...
def import_sets () :
	...
	for m_set in sets_array :
		s, created = MagicSet.objects.get_or_create(set_ID = m_set[0],
		                            set_name = m_set[1],
		                            set_release_date = m_set[2],
		                            set_num_cards = m_set[3])
		if created:
		    s.save()
...
def import_cards () :
    ...
    for card in cards_array:
        ...
        c, created = MagicCard.objects.get_or_create(card_ID = card[0],
                                    card_name = card[1].replace("#\\\\", "\\").replace("\"","'"),
                                    card_setID = s,
                                    card_type = t,
                                    card_subtype = su,
                                    ...
                                    card_loyalty = loy,
                                    card_rarity = card[8],
                                    card_text = card[9].replace("#\\\\", "\\").replace("\"","'"),
                                    card_flavor_text = card[10].replace("\\\\", "\\").replace("\"","'"),
                                    card_power = card[11],
